 ...
 */
export async function requestPhoneVerificationCode(requestId: string): Promise<string | null> {
  try {
    logger.info(`Request ${requestId} is trying to get verification code`);

    // Verificar se já existe um lock ativo
    const lockExists = await redisConnection.get(PHONE_LOCK_KEY);
    ...
    if (lockExists) {
      ...
      while (waitAttempts < maxWaitAttempts) {
        ...
        const currentLock = await redisConnection.get(PHONE_LOCK_KEY);
        if (!currentLock) {
      ...
      const finalCheck = await redisConnection.get(PHONE_LOCK_KEY);
      if (finalCheck) {
    ...
    // Adquirir o lock
    const lockAcquired = await redisConnection.set(PHONE_LOCK_KEY, requestId, 'EX', 600, 'NX');
    if (!lockAcquired) {
 ...
 */
export async function releasePhoneAccess(requestId: string): Promise<boolean> {
  try {
    logger.info(`Request ${requestId} releasing phone lock`);
 ...
 */
export async function checkPhoneLockAvailability(): Promise<boolean> {
  try {
    const lockExists = await redisConnection.get(PHONE_LOCK_KEY);
    return !lockExists;
  } catch (error) {
    logger.error(`Error checking phone lock availability:`, error);
    return false;
  }
}
 ...
 */
export async function acquirePhoneLock(requestId: string, waitForLock: boolean = false): Promise<boolean> {
  try {
    // Verificar se já existe um lock ativo
    const lockExists = await redisConnection.get(PHONE_LOCK_KEY);
    ...
    logger.info(`Request ${requestId} acquired phone lock`);
    return true;
  } catch (error) {
    logger.error(`Error acquiring phone lock for ${requestId}:`, error);
    return false;
  }
}
