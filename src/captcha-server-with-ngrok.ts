...
export class CaptchaServer {
   ...
   * This will use the ngrok URL if available, otherwise use localhost
   */
  private getBaseUrl(): string {
    try {
      // Check if we have a .ngrok-url file
      ...
      logger.warn(`Failed to read ngrok URL file: ${error}`);
    }
  ...
  public async submitCaptcha(siteKey: string, url: string): Promise<string> {
    ...
    logger.info(`Submitted captcha request ${id}, waiting for human to solve at ${baseUrl}/solve/${id}`);
